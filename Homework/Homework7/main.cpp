...
void LoadSampleText(MY_MAP &data, const std::string &filename, int window, const std::string &parse_method) {
  ...
  data[prev_prev_word][prev_word][""]+=0;
  data[prev_word][""][""]+= 0;
...
int main () {
  ...
  while (std::cin >> command) {
    ...
    if (command == "load") {
      ...
      cout << "Loaded " << filename << " with window = " << 
      window << " and parse method = " << parse_method << endl;
      cout << endl;
      ...
      for (MY_MAP::iterator i = data.begin(); i !=data.end(); ++i)
      {
        if (i->first == sentence[0])
        {
          ...
          for (map<string, map <string, int> >::iterator k = data[sentence[0]].begin(); k != data[sentence[0]].end(); ++k)
          {
            vector<int> v;
            int inner_total = 0;
            for (map <string, int>::iterator h = data[sentence[0]][k->first].begin(); h != data[sentence[0]][k->first].end(); ++h)
            {
            ...
            if (inner_total > 0)
            {
              totals2.push_back(inner_total);
            }
            else{
              totals2.push_back(data[sentence[0]][k->first].size());
            }
            
          }

          int total = 0;
          for (int b = 0; b < totals2.size(); ++b)
          {
            total += totals2[b];
          }
          ...
          for (map<string, map <string, int> >::iterator k = data[sentence[0]].begin(); k != data[sentence[0]].end(); ++k)
          {
            ...
            { 
              if (k->first !="")
              {
                cout << i-> first << ' ' << k-> first << ' ' << '(' << totals2[position] << ')' << endl;
                position ++;
              }
          ...
          if (sentence.size() == 2)
          {
            for (map<string, map <string, int> >::iterator k = data[sentence[0]].begin(); k != data[sentence[0]].end(); ++k)
            {
              if (k->first == sentence[1])
              {
                ...
                {
                  totals1.push_back(h->second);
                }
                int total1= 0;
                for (int n = 0; n < totals1.size() ; ++n)
                {
                  total1 += totals1[n];
                }
                  ...
                  for (MY_MAP::iterator l = data.begin(); l !=data.end();++l)
                  {
                    ...
                    { 
                      ...
                      for (map<string, map <string, int> >::iterator k = data[sentence[0]].begin(); k != data[sentence[0]].end(); ++k)
                      {
                        ...
                        if (totals2[position] > max)
                          {
                            max = totals2[position];
                            largest = k->first;
                          }
                        }
                        position++;
