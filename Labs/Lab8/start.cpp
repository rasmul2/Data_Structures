...
int countPaths(int x, int y, vector<vector<bool> > const& blocked_grid, vector<int> v, bool & flag){
  int count = 0;
   ...
   if (!blocked_grid[x][y]){
      if(x>= 0){
        ...
        if( y >= 0){
          v.push_back(x);
          v.push_back(y);
          count += countPaths(x, y-1,blocked_grid, v, flag);
        }
        count += countPaths(x-1, y, blocked_grid, v, flag);
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  // int x = blocked_grid.size()- start_x;
  // int y = start_y;
  // cout << x << y << endl;
  vector<int> v;
  bool flag = true;
  int answer = countPaths(start_x, start_y, blocked_grid, v, flag);
  if (answer > 1)
  {
    cout << "The number of shortest available paths is " << answer << endl;
  }
  else{
    cout << "There is " <<  answer << " shortest path" << endl;
  }
