...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    // Implement this function for Lab 13, Checkpoint 1
		...
		bool ins = true;
		hash_list_itr itr = m_table[index].begin();
		...
		m_size++;
		//return iterator and insert bool
		iterator ans_itr(this,index,itr);
		return std::make_pair(ans_itr, ins);
  }
  ...
  void resize_table(unsigned int new_size) {
    // Implement this function for Lab 13, Checkpoint 2, Part 2
		...
		for(int i=0; i<new_table.size(); ++i){
			for(hash_list_itr itr = new_table[i].begin(); itr!=new_table[i].end(); ++itr)
				this->insert(*itr);
		}
	}
};
